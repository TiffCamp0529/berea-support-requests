...
class DataLayer:
    ...
    def _ws(self,name,hdrs):
        try:
            ...
            # Ensure all expected headers exist (handles schema evolution).
            if hdrs:
    ...
    def get_tasks(self):
        if self.ok:
            try:
                ...
                # If get_all_records fails (e.g. header mismatch), try reading raw values.
                try:
                    ws=self._ws("tasks",self.TH)
                    all_vals=ws.get_all_values()
                    if len(all_vals)<2: return []
                    headers=all_vals[0]
                    return [{headers[i]:row[i] if i<len(row) else "" for i in range(len(headers))} for row in all_vals[1:] if any(c.strip() for c in row)]
                except: return []
...
def _render_task_detail(t,tid,stat,pri,uc,rec,vis):
    ...
    if cl_items and isinstance(cl_items,list):
        ...
        if cl_changed:
            DB.update_task(tid,"Checklist",json.dumps(cl_items)); st.rerun()
    hr()
    ...
    with st.expander("\U0001f5d1\ufe0f Delete this task"):
        ...
        if st.button("Confirm delete",key=f"td_{tid}"): DB.delete_task(tid); st.rerun()
...
def v_tasks():
    ...
    # Diagnostic (shows only for admin, remove once confirmed working).
    if is_admin():
        st.caption(f"📊 Task diagnostic: {len(tasks)} total from storage, {len(active)} active, {len(done)} complete.")
