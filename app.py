...
class DataLayer:
    ...
    def _ws(self,name,hdrs):
        ...
        # Step 1: get or create the worksheet. Creation only happens if it truly
        # doesn't exist — a header-sync error must NEVER reach the creation path.
        try:
            ...
            try:
                ...
                # If creation also fails (e.g. it actually exists but lookup glitched),
                # try one more plain lookup rather than crashing.
                ws=self._wb.worksheet(name)
            self._verified_ws.add(name)
            return ws
        # Step 2: sync headers once per session, isolated so any error here is harmless.
        if hdrs and name not in self._verified_ws:
            ...
            try:
                ...
                pass  # Header sync is best-effort; reads/writes still work without it.
        return ws
    def _append(self,name,hdrs,record):
        """Append a row to a sheet, always writing values in the SHEET's actual
        header order so fields never misalign — even if the schema grew over time."""
        self._invalidate_cache()
        ws=self._ws(name,hdrs)
        try:
            sheet_headers=ws.row_values(1)
            if not sheet_headers: sheet_headers=hdrs
        except: sheet_headers=hdrs
        ws.append_row([str(record.get(h,"")) for h in sheet_headers])
        return ws
    def _invalidate_cache(self):
        """Clear the per-render data snapshot so the next read fetches fresh data."""
        st.session_state["_tasks_cache_valid"]=False
        st.session_state["_subs_cache_valid"]=False
    ...
    def save_submission(self,data,ref):
        ...
        if self.ok:
            ...
            d["Details JSON"]=json.dumps(det)
            try:
                sheet_headers=ws.row_values(1)
                if not sheet_headers: sheet_headers=self.SH
            except: sheet_headers=self.SH
            ws.append_row([str(d.get(h,"")) for h in sheet_headers])
        else:
    ...
    def get_submissions(self,name=None):
        # Serve a consistent per-render snapshot; cache cleared on writes.
        if "_subs_cache" in st.session_state and st.session_state.get("_subs_cache_valid",False):
            rows=st.session_state["_subs_cache"]
        ...
        if self.ok:
            try:
                ...
                if len(all_vals)<2:
                    st.session_state["_subs_cache"]=[]; st.session_state["_subs_cache_valid"]=True
                    return []
                headers=all_vals[0]
                rows=[]
                for row in all_vals[1:]:
                    ...
                    rows.append(record)
                st.session_state["_subs_cache"]=rows; st.session_state["_subs_cache_valid"]=True
                return [r for r in rows if not name or r.get("Your Name","")==name]
            except Exception:
                rows=st.session_state.get("_subs_cache",[])
                return [r for r in rows if not name or r.get("Your Name","")==name]
    ...
    def save_checklist_template(self,key,items):
        """Save one customized checklist template (overwrites if the key exists)."""
        st.session_state.pop("_cl_templates",None)  # bust the in-session cache
        if not self.ok:
            st.session_state.setdefault("_cl_templates_local",{})[key]=items; return
        try:
            ws=self._ws("checklist_templates",["template_key","items_json"])
            cell=ws.find(key)
            if cell: ws.update_cell(cell.row,2,json.dumps(items))
            else: ws.append_row([key,json.dumps(items)])
        except: pass
                    ...
                    for row in all_vals[1:]:
                        ...
                        if rec.get("username")==user:
                            return {k:str(v).lower() not in("false","0","no","")
                                    for k,v in rec.items() if k!="username"}
            except: pass
    ...
    def save_task(self,task):
        ...
        if self.ok:
            ...
            # Write values in the SHEET's actual header order (not TH order) so
            # values always land under the correct columns even if the sheet's
            # header order differs from TH.
            try:
                sheet_headers=ws.row_values(1)
                if not sheet_headers: sheet_headers=self.TH
            except: sheet_headers=self.TH
            ws.append_row([str(task.get(h,"")) for h in sheet_headers])
        else:
            if "tasks" not in st.session_state: st.session_state.tasks=[]
            st.session_state.tasks.append(task)
    def get_tasks(self):
        # Serve one consistent snapshot per page render so every nav card and the
        # page body agree. The cache is cleared whenever a write happens.
        if "_tasks_cache" in st.session_state and st.session_state.get("_tasks_cache_valid",False):
            return st.session_state["_tasks_cache"]
        if self.ok:
            try:
                ws=self._ws("tasks",self.TH)
                all_vals=ws.get_all_values()
                if len(all_vals)<2:
                    st.session_state["_tasks_cache"]=[]; st.session_state["_tasks_cache_valid"]=True
                    return []
                headers=all_vals[0]
                tasks=[]
                for row in all_vals[1:]:
                    ...
                    record={headers[i]:(row[i] if i<len(row) else "") for i in range(len(headers))}
                    tasks.append(record)
                st.session_state["_tasks_cache"]=tasks; st.session_state["_tasks_cache_valid"]=True
                return tasks
            except Exception:
                # Read failed — return last-known-good data rather than empty,
                # so counts never spuriously drop to zero.
                return st.session_state.get("_tasks_cache",[])
    ...
    def save_delegation(self,d):
        if self.ok:
            self._append("delegations",self.DGH,d)
        else:
    ...
    def save_user(self,user):
        if self.ok:
            self._append("users",self.UH,user)
        else:
    ...
    def log_audit(self,ref_or_id,action,old_val="",new_val="",by_user="",by_role=""):
        ...
        if self.ok:
            try: self._append("audit_log",self.AH,entry)
            except: pass
        else:
    ...
    def get_audit(self,ref_or_id=None):
        ...
        if self.ok:
            ...
            rows=list(st.session_state.get("audit_log",[]))
...
def admin_sees_all():
    """Only meaningful for admins: reflects the dashboard toggle. Defaults to
    False ('My tasks') until the admin switches it to 'Everyone's tasks'."""
    return is_admin() and st.session_state.get("admin_scope_all",False)

def is_mine(t):
    """True if a task belongs to the logged-in user, or is a legacy task with no
    owner recorded. Matches on BOTH display name and email (case-insensitive) so a
    task stays 'mine' even if my display name and login path differ across sessions."""
    me=str(cur_user() or "").strip().lower()
    my_email=str(st.session_state.get("user_email","") or "").strip().lower()
    owner=str(t.get("Owner","")).strip().lower()
    assignee=str(t.get("AssignedTo","")).strip().lower()
    if not owner and not assignee: return True
    ids={x for x in (me,my_email) if x}
    return owner in ids or assignee in ids

def scope_tasks(all_tasks):
    """Apply the 'mine vs everyone' rule for the current staff user.
    - Admin with the toggle on 'Everyone' → all tasks.
    - Everyone else (and admin on 'My tasks') → only their own."""
    if admin_sees_all(): return all_tasks
    return [t for t in all_tasks if is_mine(t)]

def owner_filter_dropdown(page_key):
    """Render a 'Just mine / Everyone's' dropdown that defaults from the admin
    dashboard toggle (and to 'Just mine' for non-admins). Returns the filtered
    predicate choice as True if 'Just mine' is selected. Each page passes a
    unique page_key so the widgets don't collide."""
    default_idx=0  # Just mine
    if is_admin() and st.session_state.get("admin_scope_all",False): default_idx=1
    choice=st.selectbox("Whose items",["Just mine","Everyone's items"],
                        index=default_idx,key=f"ownfilter_{page_key}",label_visibility="collapsed")
    return choice=="Just mine"
...
# Default starter checklists per request type. Matched by a keyword in the request
# type / category string so it works whether the caller passes the full type label
# or a shorter category.
DEFAULT_CHECKLIST_TEMPLATES={
    "Event": ["Confirm date, time, and location","Reserve the room","Arrange catering",
              "Send invitations","Confirm A/V and setup needs","Prepare materials/handouts",
              "Send reminder","Confirm final headcount"],
    "Meeting": ["Confirm attendees","Check calendars for conflicts","Reserve the room",
                "Send calendar invite","Prepare and distribute agenda","Confirm A/V needs"],
    "Travel": ["Confirm travel dates","Book transportation","Reserve lodging",
               "Prepare itinerary","Submit any pre-approvals","Share confirmation details"],
    "Document": ["Gather source materials","Draft the document","Proofread and format",
                 "Route for review","Incorporate edits","Deliver final version"],
    "Communication": ["Confirm audience and purpose","Draft the message","Review tone and accuracy",
                      "Get approval if needed","Send or schedule delivery"],
    "Room": ["Confirm date and time","Check room availability","Book the room",
             "Confirm setup/A/V needs","Send confirmation"],
    "Catering": ["Confirm headcount and dietary needs","Choose menu/vendor","Place the order",
                 "Confirm delivery time and location","Confirm final count"],
    "Guest": ["Confirm visit details","Arrange parking/access","Prepare welcome materials",
              "Notify relevant staff","Confirm itinerary with guest"],
    "Budget": ["Confirm budget line and approval","Gather quotes/details","Submit purchase request",
               "Track the order","File receipts and documentation"],
    "Research": ["Clarify the research question","Identify sources","Gather and compile data",
                 "Verify accuracy","Summarize findings","Deliver report"],
    "Faculty Development": ["Confirm program details","Reserve space","Coordinate presenters",
                            "Handle registration","Prepare materials","Send reminders"],
    "Presentation": ["Confirm content and audience","Build the slides/materials","Proofread",
                     "Get feedback","Finalize and deliver"],
    "Other": ["Clarify the request","Identify steps needed","Complete the work","Review","Follow up"],
}
# Order matters for matching (check more specific keywords first).
_TEMPLATE_MATCH_ORDER=["Faculty Development","Event","Meeting","Travel","Document","Communication",
                       "Room","Catering","Guest","Budget","Research","Presentation","Other"]

def get_checklist_templates():
    """Return active templates: user-customized versions from storage if present,
    otherwise the built-in defaults."""
    stored=st.session_state.get("_cl_templates")
    if stored is not None: return stored
    if DB.ok:
        try:
            ws=DB._ws("checklist_templates",["template_key","items_json"])
            all_vals=ws.get_all_values()
            if len(all_vals)>=2:
                custom={}
                for row in all_vals[1:]:
                    if len(row)>=2 and row[0].strip():
                        try: custom[row[0]]=json.loads(row[1])
                        except: pass
                merged=dict(DEFAULT_CHECKLIST_TEMPLATES); merged.update(custom)
                st.session_state["_cl_templates"]=merged
                return merged
        except: pass
    return dict(DEFAULT_CHECKLIST_TEMPLATES)

def match_template(type_or_category):
    """Find the best-matching template for a request-type or category string."""
    templates=get_checklist_templates(); s=str(type_or_category)
    for key in _TEMPLATE_MATCH_ORDER:
        if key.lower() in s.lower() and key in templates:
            return templates[key]
    return templates.get("Other",DEFAULT_CHECKLIST_TEMPLATES["Other"])
...
def sec(l):
    # A real heading (not just a styled div) so screen-reader users can jump
    # between these section labels the same way sighted users scan for them visually.
    st.markdown(f'<h3 class="sec">{l}</h3>',unsafe_allow_html=True)
...
def gen_ref():
    # Timestamp + a short random suffix so two requests landing in the same
    # second (two supervisors submitting at once, etc.) never collide — a
    # collision here would double as a duplicate Streamlit widget key and
    # crash the page, and would confuse row lookups in Google Sheets.
    return f"REQ-{datetime.now().strftime('%Y%m%d-%H%M%S')}-{uuid.uuid4().hex[:4]}"
def gen_task_id():
    return f"TASK-{datetime.now().strftime('%Y%m%d-%H%M%S')}-{uuid.uuid4().hex[:4]}"
        ...
        if submitted:
            ...
            if sheet_user and DB._hash_pw(password)==sheet_user.get("password_hash",""):
                ...
                if password==ADMIN_PASSWORD and (not admin_email or em==admin_email or em==""):
                    # Use the SAME display name the sheet would use, so ownership matches
                    # regardless of which login path is taken.
                    seeded=DB.get_user_by_email(admin_email) if admin_email else None
                    admin_display=(seeded.get("display_name") if seeded else "") or ADMIN_NAME
                    st.session_state.user=admin_display; st.session_state.user_role="admin"
...
def v_form():
    ...
    # Build the list of supervisor names from BOTH the users sheet and secrets.
    sheet_sups={}  # name -> {role, email}
    for u in DB.get_users():
        if u.get("role","")=="supervisor" and str(u.get("active","True")).lower()!="false":
            nm=u.get("display_name","")
            if nm: sheet_sups[nm]={"role":u.get("title",""),"email":u.get("email","")}
    # Merge with secrets-based supervisors (users sheet takes priority).
    merged_sups=dict(SUPERVISORS); merged_sups.update(sheet_sups)
    sup_names=sorted(merged_sups.keys())
    ...
    with c2:
        ...
        auto_role=merged_sups.get(name_choice,{}).get("role","") if name_choice in sup_names else ""
    ...
    default_contact=merged_sups.get(name,{}).get("email","") if name in sup_names else ""
    ...
    st.caption("These are your account-wide email settings (the same ones under Notification Settings) — "
               "shown here so you can adjust them without leaving the form.")
    # Load the submitter's EXISTING saved preferences as the defaults here, instead of
    # hardcoding both boxes to checked. Otherwise, submitting a new request would
    # silently re-enable emails for anyone who'd previously turned them off in
    # Settings, since this form saves to that same preferences record.
    _existing_prefs=DB.get_prefs(name) if name.strip() else dict(DB.PD)
    n_stat=st.checkbox("Email me when the status of this request changes",value=_existing_prefs.get("on_status_change",True),key="pf_st")
    n_note=st.checkbox("Email me when a note is added to this request",value=_existing_prefs.get("on_new_note",True),key="pf_nt")
    ...
    if st.button("✅  Submit Support Request",key="sub_btn"):
        ...
        # Merge into (not overwrite) the submitter's saved preferences — keeps
        # on_new_submission/on_any_note as they already were rather than resetting
        # them, and only updates the two boxes shown on this form.
        DB.save_prefs(name,{**_existing_prefs,"on_new_note":n_note,"on_status_change":n_stat})
...
def generate_ai_checklist(sub):
    """Generate a checklist for this request. Uses AI when an API key is present;
    otherwise falls back to a type-based template. In all cases, any checklist items
    the supervisor already provided are placed FIRST, then the generated items."""
    # Collect supervisor-provided items (these always go first).
    sup_items=[]
    sup_cl=sub.get("Checklist","")
    if sup_cl:
        try:
            for i in json.loads(sup_cl):
                txt=i.get("text","") if isinstance(i,dict) else str(i)
                if txt.strip(): sup_items.append({"text":txt.strip(),"done":False})
        except: pass
    rt=sub.get("Request Type","") or sub.get("Category","")
    api_key=_s("ANTHROPIC_API_KEY","")
    generated=[]
    if api_key:
        ai=_ai_checklist(sub,api_key)
        if ai: generated=ai
    if not generated:
        # Template fallback — matched by request type / category.
        tmpl=match_template(rt)
        generated=[{"text":item,"done":False} for item in tmpl]
    # Merge supervisor items first, then generated, skipping duplicates (case-insensitive).
    seen=set(); merged=[]
    for it in sup_items+generated:
        key=it["text"].strip().lower()
        if key and key not in seen:
            seen.add(key); merged.append(it)
    return merged if merged else None

def _ai_checklist(sub,api_key):
    """The AI-powered path (only called when an API key exists). Returns a list of
    {text,done} dicts or None on any failure."""
    try:
        ...
        dj=sub.get("Details JSON",""); extra=""
        if dj:
            try:
                ex=json.loads(dj)
                extra="\n".join([f"{k}: {v}" for k,v in ex.items() if v and str(v) not in ("","—","False")])
            except: pass
        sup_cl=sub.get("Checklist",""); sup_note=""
        if sup_cl:
            try:
...
def accept_request(sub):
    ...
    DB.save_task({"task_id":gen_task_id(),
        ...
        "Owner":accepter,"AssignedTo":accepter,"StaffVisible":"Yes"})
...
def admin_nav_cards(current=""):
    ...
    # Scope tasks to the current user (admins with the toggle on 'Everyone' see all).
    my_tasks=scope_tasks(tasks)
    active_tasks=[t for t in my_tasks if t.get("Status","") not in ("Complete","")]
    done_ct=sum(1 for t in my_tasks if t.get("Status","")=="Complete")
    # Pending acceptance is ALWAYS the full shared queue — unclaimed work anyone can take.
    pending_reqs=[s for s in subs
                  if s.get("Status","") in ("New","Under Review","")
                  and s.get("ref_number","") not in converted_refs]
    # Urgent/high = my urgent/high active tasks + the shared urgent/high pending requests.
    ...
    with c0:
        ...
        if st.button(("\u2022 You are here" if is_home else "Dashboard \u2192"),key=f"nav_dashboard_{current}",use_container_width=True,disabled=is_home):
            st.session_state.view="admin"; st.rerun()
    # Labels adapt when an admin is viewing everyone's tasks.
    seeing_all=admin_sees_all()
    plate_label="All active tasks" if seeing_all else "On my plate"
    done_label="All completed" if seeing_all else "Completed"
    _card(c1,len(active_tasks),plate_label,"bl","My tasks \u2192","tasks")
    _card(c2,len(pending_reqs),"Pending acceptance","bl","Review \u2192","pending")
    _card(c3,urg_ct,"Urgent / high","am","View urgent \u2192","urgent")
    _card(c4,done_ct,done_label,"gr","Completed \u2192","completed")

# ── View: Admin dashboard ────────────────────────────────────────────────────
def v_admin():
    banner("Admin Dashboard — Everything on My Plate","Berea College · Office of the Provost · Admin view")
    # Admin-only scope toggle: My tasks (default) vs Everyone's tasks. Drives both
    # the card counts and the task lists across pages.
    if is_admin():
        tog1,tog2=st.columns([2,3])
        with tog1:
            choice=st.radio("Show",["My tasks","Everyone's tasks"],
                            index=(1 if st.session_state.get("admin_scope_all",False) else 0),
                            horizontal=True,key="admin_scope_radio",label_visibility="collapsed")
            st.session_state.admin_scope_all=(choice=="Everyone's tasks")
        st.caption(f"Showing counts and lists for: **{choice}**. "
                   f"Switch anytime — this affects what you see across all pages.")
    subs=DB.get_submissions()
    tasks=DB.get_tasks()
    ...
    for sub in filt:
        ...
        # Expander labels are plain text (no HTML), so the confidential flag has to be a
        # plain marker in the title itself to actually show up in the collapsed list.
        cf="🔒 " if conf else ""
        short=ref.split("-")[-1] if ref else "—"
        _conv=any(t.get("source_ref","")==ref for t in tasks)
        _acc="✅ " if _conv else ""
        with st.expander(f"{_acc}{cf}{'🔴 ' if 'Urgent' in urg else '🟠 ' if 'High' in urg else ''}{short}  ·  {name}  ·  {rt}  ·  {stat}"):
            ...
            with cr:
                ...
                # Only genuinely new/unreviewed requests get Accept/Deny — not just
                # "no task currently exists." Otherwise, deleting the task created by
                # an earlier acceptance would bring the buttons back and let you
                # accept the same request twice (a second task + a duplicate "accepted"
                # email to the submitter).
                if stat in ("New","Under Review","") and not already_converted:
...
def view_pending():
    ...
    for sub in sorted(pending,key=lambda x:x.get("submitted_at","")):
        ...
        conf="Yes" in str(sub.get("Confidential",""))
        cf="🔒 " if conf else ""
        with st.expander(f"{cf}{'🔴 ' if 'Urgent' in urg else '🟠 ' if 'High' in urg else ''}{name}  ·  {rt}  ·  {sub.get('Deadline','')}"):
            _render_request_details(sub)
            hr()
            b1,b2=st.columns(2)
            with b1:
...
def view_urgent():
    ...
    tasks=DB.get_tasks()
    converted_refs={t.get("source_ref","") for t in tasks if t.get("source_ref","").strip()}
    uc1,uc2=st.columns([1.3,3.7])
    with uc1: mine_only=owner_filter_dropdown("urgent")
    scoped=[t for t in tasks if is_mine(t)] if mine_only else tasks
    u_tasks=[t for t in scoped if t.get("Status","") not in ("Complete","")
...
def v_portal():
    ...
    if not shared_active:
        ...
        for tk in sorted(shared_active,key=lambda x:x.get("Deadline","")):
            ...
            tsc=S_CSS.get(tstat,"s-new"); tuc=task_priority_css(tpri)
...
def add_task_form():
    ...
    sec("➕ Add a New Task")
    pii_banner(compact=True)
    # Build the list of staff you could assign a task to (admin/colleague/support, not supervisors).
    me=cur_user(); my_email=st.session_state.get("user_email","")
    other_staff=[u for u in DB.get_users()
                 if u.get("role","") in ("admin","colleague","support")
                 and str(u.get("active","True")).lower()!="false"
                 and str(u.get("email","")).strip().lower()!=my_email.strip().lower()]
    assign_options=["Myself"]+[f'{u.get("display_name","")} ({u.get("role","")})' for u in other_staff]
    # Requestor is always the logged-in user (auto-filled, shown read-only).
    st.markdown(f'<div style="font-size:.82rem;margin-bottom:6px;">'
                f'<strong>Requestor:</strong> {me} '
                f'<span style="color:#5a7a96;">(that\'s you — filled in automatically)</span></div>',
                unsafe_allow_html=True)
    with st.form("new_task",clear_on_submit=True):
        ...
        # Assign-to dropdown (defaults to Myself).
        t_assign=st.selectbox(lbl("Assign to"),assign_options,
                              help="Keep as 'Myself' to add this to your own list. Choose someone else to send it as a request they'll accept or decline.")
        ...
        if st.form_submit_button("➕ Add task"):
            if not t_title.strip():
            ...
            elif t_assign=="Myself":
                # Create the task directly on my own list.
                tid=gen_task_id()
                DB.save_task({"task_id":tid,"created_at":datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
                    ...
                    "Owner":me,"AssignedTo":me,"StaffVisible":"Yes"})
                DB.log_audit(tid,"task_created","",t_title.strip(),me,st.session_state.get("user_role",""))
                st.success(f"Task added to your list: {t_title.strip()}"); st.rerun()
            else:
                # Send it as a delegation request to the chosen staff member.
                chosen=other_staff[assign_options.index(t_assign)-1]  # -1 because "Myself" is index 0
                import uuid
                did=str(uuid.uuid4())[:8]
                DB.save_delegation({"deleg_id":did,"created_at":datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
                    "from_user":me,"to_email":chosen.get("email",""),
                    "to_name":chosen.get("display_name",""),
                    "Title":t_title.strip(),"Category":t_cat,"Priority":t_pri,
                    "Deadline":t_ddl.strftime("%Y-%m-%d"),"Details":t_det.strip(),
                    "Status":"Pending","task_id":""})
                DB.log_audit(did,"delegation_sent","",chosen.get("display_name",""),me,st.session_state.get("user_role",""))
                st.success(f"Task request sent to {chosen.get('display_name','')}. "
                           f"They'll see it under their incoming task requests and can accept or decline.")
                st.rerun()
...
def _render_request_details(sub):
    ...
    if dj:
        try:
            ex=json.loads(dj)
            if ex:
                # Internal plumbing we never show as request content.
                ...
                def _is_empty(v):
                    # Truly-empty values we hide; but NOT False/0, which are real answers.
                    if v is None: return True
                    s=str(v).strip()
                    return s in ("","—","{}","[]","None")
                def _fmt(v):
                    # Make explicit answers read naturally.
                    if v is True or str(v)=="True": return "Yes"
                    if v is False or str(v)=="False": return "No"
                    return v
                items=[(k,_fmt(v)) for k,v in ex.items() if not _is_empty(v) and k not in skip_keys]
                if items:
...
def _render_task_detail(t,tid,stat,pri,uc,rec,vis):
    ...
    # If this task came from a request, show the full original request details.
    if src:
        linked=[s for s in DB.get_submissions() if s.get("ref_number","")==src]
        if linked:
            with st.expander("📄 View the original request this task came from"):
                _render_request_details(linked[0])
    ...
    if cl_items and isinstance(cl_items,list):
        if editable:
            ...
            for ci,item in enumerate(cl_items):
                ...
                with rm_col:
                    if st.button("✕",key=f"clrm_{tid}_{ci}",help="Remove this item"):
                        remove_idx=ci
            if remove_idx is not None:
                st.session_state.open_task_tid=tid
                removed=cl_items.pop(remove_idx)
                DB.update_task(tid,"Checklist",json.dumps(cl_items))
                DB.log_audit(tid,"checklist_item_removed","",removed.get("text",""),cur_user(),st.session_state.get("user_role",""))
                st.rerun()
            if cl_changed:
                st.session_state.open_task_tid=tid  # keep expander open after rerun
                DB.update_task(tid,"Checklist",json.dumps(cl_items))
            ...
            for item in cl_items:
                icon="✅" if item.get("done",False) else "⬜"
                st.markdown(f'{icon} {item.get("text","")}')
    # Add-a-checklist-item row (only for people who can edit the task).
    if editable:
        ac1,ac2=st.columns([5,1])
        with ac1:
            new_item=st.text_input("Add a checklist item",key=f"clnew_{tid}",
                                   placeholder="Type a new step and click Add",label_visibility="collapsed")
        with ac2:
            if st.button("Add",key=f"cladd_{tid}",use_container_width=True):
                if new_item.strip():
                    st.session_state.open_task_tid=tid
                    cur=cl_items if (cl_items and isinstance(cl_items,list)) else []
                    cur.append({"text":new_item.strip(),"done":False})
                    DB.update_task(tid,"Checklist",json.dumps(cur))
                    DB.log_audit(tid,"checklist_item_added","",new_item.strip(),cur_user(),st.session_state.get("user_role",""))
                    st.rerun()
    hr()
    if not editable:
    ...
    # Quick actions row. Hide the "Full detail view" button when we're already on
    # the detail page (avoids a redundant button that reloads the same page).
    on_detail_page=(st.session_state.get("view","")=="task_detail")
    if on_detail_page:
        ics=generate_ics(t)
        if ics:
...
def task_priority_css(pri):
    """Badge color for a TASK's priority (Urgent/High/Medium/Low). Kept separate
    from ucss(), which is for REQUEST urgency (Urgent/High/Standard/Low) — the two
    scales use different words for the middle tier, so they must not be mixed."""
    return ("urg-u" if "Urgent" in pri else "urg-h" if "High" in pri else "urg-s" if "Medium" in pri else "urg-l")

def _task_props(t):
    tid=t.get("task_id",""); pri=t.get("Priority","")
    uc=task_priority_css(pri)
...
def _repair_task_sheet():
    """Rebuild the tasks sheet with canonical header order, preserving data by
    mapping each existing row's values by its CURRENT header names. Fixes any
    column misalignment introduced by schema changes over time."""
    if not DB.ok:
        st.warning("Repair only works with Google Sheets storage."); return
    try:
        ws=DB._ws("tasks",DB.TH)
        raw=ws.get_all_values()
        if len(raw)<1:
            st.info("Sheet is empty — nothing to repair."); return
        old_headers=raw[0]
        # Build dict records using the sheet's current header order.
        records=[]
        for row in raw[1:]:
            if not any(str(c).strip() for c in row): continue
            rec={old_headers[i]:(row[i] if i<len(row) else "") for i in range(len(old_headers))}
            records.append(rec)
        # Clear the sheet and rewrite with canonical TH headers + remapped rows.
        ws.clear()
        ws.append_row(DB.TH)
        for rec in records:
            ws.append_row([str(rec.get(h,"")) for h in DB.TH])
        # Reset the verified cache so headers re-sync cleanly next read.
        if hasattr(DB,"_verified_ws"): DB._verified_ws.discard("tasks")
        st.success(f"✅ Repaired. Rebuilt {len(records)} task rows with correct column alignment. Refresh the page.")
    except Exception as e:
...
def v_tasks():
    ...
    if is_admin():
        ...
        with st.expander("🔧 Storage diagnostic (admin) — check task data health"):
            if DB.ok:
                try:
                    ws=DB._ws("tasks",DB.TH)
                    raw=ws.get_all_values()
                    if raw:
                        st.markdown("**Sheet header row (actual order in Google Sheets):**")
                        st.code(" | ".join(raw[0]))
                        st.markdown(f"**Expected headers (app's TH order):**")
                        st.code(" | ".join(DB.TH))
                        # Detect obvious problems.
                        problems=[]
                        if "Status" not in raw[0]: problems.append("❌ No 'Status' column in sheet header!")
                        if raw[0].count("Status")>1: problems.append("❌ Duplicate 'Status' columns!")
                        for h in DB.TH:
                            if h not in raw[0]: problems.append(f"⚠️ Missing column: {h}")
                        # Check a sample data row for a valid status.
                        if len(raw)>=2:
                            hdr=raw[0]; sample=raw[1]
                            si=hdr.index("Status") if "Status" in hdr else -1
                            sval=sample[si] if 0<=si<len(sample) else "(none)"
                            st.markdown(f"**First task's Status value reads as:** `{sval}`")
                            valid_statuses=set(STATUS_OPTIONS)
                            if sval and sval not in valid_statuses:
                                problems.append(f"❌ First task's Status '{sval}' isn't a valid status — columns may be misaligned.")
                        if problems:
                            st.error("Problems detected:\n\n"+"\n\n".join(problems))
                            st.markdown("If columns are misaligned, click below to rebuild the sheet with correct headers. "
                                        "**This preserves your data** by re-mapping each row to the right columns.")
                            if st.button("🔧 Repair task sheet alignment"):
                                _repair_task_sheet()
                        else:
                            st.success("✅ Task sheet headers look healthy and aligned.")
                except Exception as e:
                    st.warning(f"Couldn't read raw sheet: {e}")
    ...
    with tc4:
        # Default the owner filter from the admin dashboard toggle; non-admins default to "Just mine".
        own_opts=["Just mine","Everyone's tasks"]
        default_own=0  # Just mine
        if is_admin() and st.session_state.get("admin_scope_all",False): default_own=1
        fown=st.selectbox("Whose tasks",own_opts,index=default_own,key="tf_own",label_visibility="collapsed")
    shown=active
    ...
    if fown=="Just mine":
        shown=[t for t in shown if is_mine(t)]
                ...
                with cols[ci]:
                    ...
                    for t in sorted(status_groups[stat_name],key=lambda x:x.get("Deadline","")):
                        ...
                        with st.expander(f"Edit: {title[:25]}\u2026" if len(title)>25 else f"Edit: {title}",expanded=False):
                            _render_task_detail(t,tid,stat,pri,uc,rec,vis)
                        if st.button("Open →",key=f"cardopen_{tid}",use_container_width=True,
                                     help="Open this task on its own full-detail page"):
                            st.session_state.detail_task_id=tid; st.session_state.view="task_detail"; st.rerun()
    else:
        sec("\U0001f4cb Active Tasks")
        if not shown: st.caption("No active tasks match your filters.")
        open_tid=st.session_state.get("open_task_tid","")
        for t in sorted(shown,key=lambda x:x.get("Deadline","")):
            tid,pri,uc,stat,vis,rec=_task_props(t)
            lc1,lc2=st.columns([5,1])
            with lc1:
                with st.expander(f"{pri.split(' ')[0]} {t.get('Title','')}  \u00b7  {t.get('Category','')}  \u00b7  {stat}",
                                 expanded=(tid==open_tid)):
                    _render_task_detail(t,tid,stat,pri,uc,rec,vis)
            with lc2:
                if st.button("Open →",key=f"listopen_{tid}",use_container_width=True,
                             help="Open this task on its own full-detail page"):
                    st.session_state.detail_task_id=tid; st.session_state.view="task_detail"; st.rerun()
    ...
    if fown=="Just mine": done_filtered=[t for t in done_filtered if is_mine(t)]
    ...
    # ── Delegated by me (tasks I sent to other staff) ──
    my_delegations=[d for d in DB.get_delegations() if d.get("from_user","")==cur_user()]
    if my_delegations:
        st.markdown("<br>",unsafe_allow_html=True)
        sec("\U0001f4e4 Delegated by me")
        st.caption("Tasks you've sent to other staff. These live on their task lists once accepted — "
                   "shown here so you can track what you've handed off.")
        for d in sorted(my_delegations,key=lambda x:x.get("created_at",""),reverse=True):
            stat=d.get("Status","Pending")
            icon={"Pending":"\u23f3","Accepted":"\u2705","Declined":"\U0001f6ab"}.get(stat,"\u23f3")
            color={"Pending":"#C75B12","Accepted":"#27500A","Declined":"#999"}.get(stat,"#C75B12")
            pri=d.get("Priority",""); pi=pri.split(" ")[0] if pri else ""
            st.markdown(f'<div style="background:#fff;border:1px solid #dde3eb;border-left:4px solid {color};'
                        f'border-radius:0 6px 6px 0;padding:8px 12px;margin-bottom:6px;font-size:.82rem;">'
                        f'{icon} <strong>{d.get("Title","")}</strong> {pi} &nbsp;\u00b7&nbsp; '
                        f'sent to <strong>{d.get("to_name","")}</strong> &nbsp;\u00b7&nbsp; '
                        f'due {d.get("Deadline","")} &nbsp;\u00b7&nbsp; '
                        f'<span style="color:{color};font-weight:700;">{stat}</span></div>',unsafe_allow_html=True)
    add_task_form()
...
def v_today():
    ...
    st.markdown("<br>",unsafe_allow_html=True)
    tasks=DB.get_tasks()
    tdc1,tdc2=st.columns([1.3,3.7])
    with tdc1: mine_only=owner_filter_dropdown("today")
    if mine_only: tasks=[t for t in tasks if is_mine(t)]
    ...
    if tasks_without_checklist:
        ...
        for t in sorted(tasks_without_checklist,key=lambda x:x.get("Deadline","")):
            ...
            with wc2:
                if st.button("Open →",key=f"twc_open_{t.get('task_id','')}",use_container_width=True):
                    st.session_state.detail_task_id=t.get("task_id",""); st.session_state.view="task_detail"; st.rerun()
...
def v_planner():
    ...
    st.markdown("<br>",unsafe_allow_html=True)
    tasks=DB.get_tasks()
    plc1,plc2=st.columns([1.3,3.7])
    with plc1: mine_only=owner_filter_dropdown("planner")
    if mine_only: tasks=[t for t in tasks if is_mine(t)]
    active=[t for t in tasks if t.get("Status","") not in ("Complete","")]
    today=date.today()
    # Week selector.
    ...
    for di,d in enumerate(days):
        ...
        with day_cols[di]:
            ...
            # Built as a plain string, not inside an f-string {...} expression — a
            # backslash-escaped quote inside an f-string's {} is a SyntaxError on
            # Python versions before 3.12, which would stop the whole app from
            # starting on Streamlit Cloud's default Python.
            today_tag='<div style="font-size:.65rem;font-weight:700;color:#C75B12;">TODAY</div>' if is_today else ""
            ...
            if day_tasks:
                for t in day_tasks:
                    ...
                    st.markdown(f'<div style="background:#fff;border:1px solid #dde3eb;border-left:3px solid {BLUE};'
                                f'border-radius:0 6px 6px 0;padding:6px 8px;margin-bottom:2px;font-size:.75rem;">'
                    ...
                    if st.button("Open →",key=f"plan_open_{t.get('task_id','')}_{d_str}",use_container_width=True):
                        st.session_state.detail_task_id=t.get("task_id",""); st.session_state.view="task_detail"; st.rerun()
    ...
    if overdue:
        ...
        for t in sorted(overdue,key=lambda x:x.get("Deadline","")):
            ...
            with oc2:
                if st.button("Open →",key=f"od_open_{t.get('task_id','')}",use_container_width=True):
                    st.session_state.detail_task_id=t.get("task_id",""); st.session_state.view="task_detail"; st.rerun()
    # Upcoming (next week+).
...
def v_task_detail():
    ...
    if not tid:
        ...
        if st.button("← Back to My Tasks",key="detail_back_none"):
            st.session_state.view="tasks"; st.rerun()
        return
    tasks=DB.get_tasks()
    task=[t for t in tasks if t.get("task_id","")==tid]
    if not task:
        st.warning("That task couldn't be found — it may have been deleted or the list is still loading.")
        if st.button("← Back to My Tasks",key="detail_back_nf"):
            st.session_state.view="tasks"; st.rerun()
        return
    t=task[0]
    title=t.get("Title","(untitled)")
    banner(f"Task Detail: {title}","Berea College · Office of the Provost")
    # Clear context banner so it's obvious this is a single-task page, not a lost list.
    st.markdown(
        f'<div style="background:#eef4fb;border-left:4px solid {BLUE};border-radius:0 8px 8px 0;'
        f'padding:10px 14px;margin:4px 0 10px;font-size:.86rem;">'
        f'📄 You\'re viewing <strong>one task</strong> in full. Your other tasks are still here — '
        f'use the button below to go back to the full list.</div>',unsafe_allow_html=True)
    if st.button("← Back to all my tasks",key="detail_back",use_container_width=False):
        st.session_state.view="tasks"; st.rerun()
    st.markdown("<br>",unsafe_allow_html=True)
    ...
    try:
        ...
        st.error(f"Something went wrong displaying this task's details: {e}")
        st.caption("The task itself is safe. Please use the back button and try again, "
                   "or report this message if it keeps happening.")
    # ── Generate checklist (if none exists) ──
    cl_raw=t.get("Checklist","")
    if not cl_raw or cl_raw=="[]":
        st.markdown("<br>",unsafe_allow_html=True)
        sec("📋 Generate Checklist")
        st.caption("This task doesn't have a checklist yet. Auto-generate a starter checklist "
                   "based on the request type — then edit, add, or remove items as needed.")
        if st.button("🪄 Auto-generate checklist",key=f"gen_cl_{tid}"):
            with st.spinner("Generating checklist..."):
                ...
                if checklist:
                    ...
                    DB.log_audit(tid,"checklist_generated","",f"{len(checklist)} items",cur_user(),st.session_state.get("user_role",""))
                    st.success(f"Added {len(checklist)} starter items. Edit or add to them below."); st.rerun()
                else:
                    st.info("Couldn't build a checklist automatically — you can add items manually below.")
    ...
    if not active_users:
        ...
        for u in sorted(active_users,key=lambda x:x.get("display_name","")):
            ...
            with st.expander(f'{role_icon} {u.get("display_name","")} — {u.get("email","")} ({role})'):
                ...
                with nc3:
                    new_email=st.text_input("Change email (= their login)",value=u.get("email",""),key=f"uem_{uid}")
                st.caption("⚠️ Changing the email changes how this person signs in. Tell them their new login email if you change it.")
                if st.button("💾 Save changes",key=f"us_{uid}"):
                    old_email=u.get("email","")
                    msg=[]; err=None
                    ne=new_email.strip().lower()
                    if ne and ne!=old_email.strip().lower():
                        # Prevent collision with another active user's email.
                        clash=[x for x in active_users if str(x.get("email","")).strip().lower()==ne and x.get("user_id","")!=uid]
                        if clash:
                            err=f"That email is already used by {clash[0].get('display_name','another user')}."
                        elif "@" not in ne:
                            err="Please enter a valid email address."
                        else:
                            DB.update_user(uid,"email",new_email.strip())
                            DB.log_audit(uid,"email_change",old_email,new_email.strip(),cur_user(),st.session_state.user_role)
                            msg.append(f"email changed to {new_email.strip()}")
                    if err:
                        st.error(f"⚠️ {err}")
                    else:
                        if new_role!=role:
                            ...
                            msg.append(f"role changed to {new_role}")
                        if new_pw.strip():
                            ...
                            msg.append("password reset")
                        # If the admin edited their OWN email, refresh the session so they stay logged in.
                        if old_email.strip().lower()==st.session_state.get("user_email","").strip().lower() and ne and ne!=old_email.strip().lower():
...
def v_prefs():
    ...
    sec("📧 Change Your Email")
    if not u_record:
        st.caption("Your account isn't editable here (it's configured at the system level). "
                   "Ask an admin if you need your email changed.")
    else:
        st.markdown('<div class="ro-notice">⚠️ Your email address is also your <strong>login username</strong>. '
                    'If you change it here, you\'ll need to sign in with the new email next time.</div>',
                    unsafe_allow_html=True)
        with st.form("email_change_form",clear_on_submit=False):
            new_email=st.text_input("New email address",value=u_record.get("email",""),key="em_new")
            confirm_pw=st.text_input("Confirm with your current password",type="password",
                                     placeholder="Enter your password to confirm this change",key="em_pw")
            if st.form_submit_button("Update email"):
                errs=[]
                ne=new_email.strip().lower()
                old=u_record.get("email","").strip().lower()
                if not ne: errs.append("Email can't be blank.")
                elif "@" not in ne: errs.append("Please enter a valid email address.")
                if ne==old: errs.append("That's already your email — no change needed.")
                if not confirm_pw.strip(): errs.append("Please enter your current password to confirm.")
                elif DB._hash_pw(confirm_pw)!=u_record.get("password_hash",""):
                    errs.append("Password is incorrect.")
                # Prevent collision with another active user.
                if not errs:
                    others=[x for x in DB.get_users()
                            if str(x.get("email","")).strip().lower()==ne
                            and x.get("user_id","")!=u_record.get("user_id","")
                            and str(x.get("active","True")).lower()!="false"]
                    if others: errs.append("That email is already in use by another account.")
                if errs:
                    for e in errs: st.error(f"⚠️ {e}")
                else:
                    DB.update_user(u_record.get("user_id",""),"email",new_email.strip())
                    DB.log_audit(u_record.get("user_id",""),"email_change",u_record.get("email",""),
                                 new_email.strip(),user,st.session_state.user_role)
                    st.session_state.user_email=new_email.strip()  # keep session valid immediately
                    st.success(f"✅ Email updated to {new_email.strip()}. "
                               f"Use this new email to sign in next time.")
                    st.rerun()
    ...
    if is_admin():
        hr(); sec("📋 Checklist Templates")
        st.caption("These are the starter checklists used when you auto-generate a checklist for a task. "
                   "Edit them here to match how your office works — changes apply to future auto-generated checklists. "
                   "Put one item per line.")
        templates=get_checklist_templates()
        tkeys=list(DEFAULT_CHECKLIST_TEMPLATES.keys())
        pick=st.selectbox("Which template to edit",tkeys,key="tmpl_pick")
        current_items=templates.get(pick,DEFAULT_CHECKLIST_TEMPLATES.get(pick,[]))
        edited=st.text_area(f"Items for “{pick}” (one per line)",
                            value="\n".join(current_items),height=200,key=f"tmpl_edit_{pick}")
        bcol1,bcol2=st.columns([1,1])
        with bcol1:
            if st.button("💾 Save this template",key=f"tmpl_save_{pick}",use_container_width=True):
                items=[ln.strip() for ln in edited.split("\n") if ln.strip()]
                if items:
                    DB.save_checklist_template(pick,items)
                    st.success(f"Saved the “{pick}” template ({len(items)} items).")
                else:
                    st.warning("A template needs at least one item.")
        with bcol2:
            if st.button("↩ Reset to default",key=f"tmpl_reset_{pick}",use_container_width=True):
                DB.save_checklist_template(pick,DEFAULT_CHECKLIST_TEMPLATES[pick])
                st.success(f"Reset “{pick}” to the built-in default."); st.rerun()
...
def main():
    # Fresh data snapshot each render pass: invalidate the cache so the first
    # read this run fetches live data, then subsequent reads in the same run
    # (e.g. all the nav cards) reuse that identical snapshot for consistent counts.
    st.session_state["_tasks_cache_valid"]=False
    st.session_state["_subs_cache_valid"]=False
