...
def admin_nav_cards(current=""):
    ...
    pending_reqs=[s for s in subs if s.get("Status","") not in ["Complete","Declined","Cancelled"]
...
def view_pending():
    ...
    tasks=DB.get_tasks()
    converted_refs={t.get("source_ref","") for t in tasks if t.get("source_ref","")}
    pending=[s for s in subs if s.get("Status","") not in ["Complete","Declined","Cancelled"]
             and s.get("ref_number","") not in converted_refs]
...
def view_urgent():
    ...
    u_reqs=[s for s in subs if s.get("Status","") not in ["Complete","Declined","Cancelled"]
            and s.get("ref_number","") not in converted_refs
