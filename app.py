...
def is_mine(t):
    """True if a task belongs to the logged-in user (owner or assignee), or is a
    legacy task with no owner recorded (so nothing gets stranded)."""
    me=cur_user()
    owner=str(t.get("Owner","")).strip()
    assignee=str(t.get("AssignedTo","")).strip()
    if not owner and not assignee: return True
    return me==owner or me==assignee
