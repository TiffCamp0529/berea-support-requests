    ...
    def get_users(self,raw=False):
        ...
        if self.ok:
            try:
                ...
                try:
                    ws=self._ws("users",self.UH); all_vals=ws.get_all_values()
                    if len(all_vals)>=2:
                        headers=all_vals[0]
                        records=[{headers[i]:row[i] if i<len(row) else "" for i in range(len(headers))} for row in all_vals[1:] if any(c.strip() for c in row)]
                except: records=[]
        else:
