...
class DataLayer:
    ...
    def get_users(self):
        if self.ok:
            try: return self._ws("users",self.UH).get_all_records()
            except: return []
        return list(st.session_state.get("app_users",[]))
    ...
    def seed_users_from_secrets(self):
        """On first run, migrate admin + supervisor accounts from secrets into the
        users sheet so everything lives in one place going forward."""
...
DB=get_db()
# Seed users from secrets on first run (idempotent — skips if already present).
if DB.ok:
    try: DB.seed_users_from_secrets()
    except: pass
...
def is_admin_or_support(): return st.session_state.user_role in ("admin","support")
...
def accept_request(sub):
    ...
    if not checklist:
        # Fall back to supervisor-provided checklist if AI isn't available.
        sup_cl=sub.get("Checklist","")
        if sup_cl:
...
def admin_nav_cards(current=""):
    """Render the four count cards + navigation buttons at the top of any admin
    page. `current` is the view name of the page we're on, so its button is
    styled as the active one."""
    subs=DB.get_submissions()
    tasks=DB.get_tasks()
    ...
    def _card(col,num,label,cs,btn_label,target_view):
        with col:
            ...
            here=(target_view==current) or (current=="admin" and target_view=="dashboard")
            lbl_txt=("• You are here" if here else btn_label)
            if st.button(lbl_txt,key=f"nav_{target_view}_{current}",use_container_width=True):
                st.session_state.view=target_view; st.rerun()
    c0,c1,c2,c3,c4=st.columns(5)
    # First card returns to the main dashboard.
    with c0:
        st.markdown(f'<div class="mc"><div class="mn bl">🏠</div><div class="ml">Dashboard</div></div>',unsafe_allow_html=True)
        if st.button(("• You are here" if current in ("admin","dashboard") else "Dashboard →"),
                     key=f"nav_dashboard_{current}",use_container_width=True):
            st.session_state.view="admin"; st.rerun()
...
def v_portal():
    ...
    st.caption(f"You have submitted **{total}** request{'s' if total!=1 else ''} in total. Each one is counted in exactly one card above based on its current status — so you can always see where things stand at a glance."
               f"{' (Plus ' + str(sum(1 for s in subs if s.get('Status','') in ('Cancelled','Declined'))) + ' cancelled/declined — these still appear in your list below for your records.)' if any(s.get('Status','') in ('Cancelled','Declined') for s in subs) else ''}")
    ...
    for sub in sorted(subs,key=lambda x:x.get("submitted_at",""),reverse=True):
        ...
        with st.expander(f"{short}  ·  {rt}  ·  {fstat}  ·  {f'{nct} note(s)' if nct else 'No notes'}"):
            ...
            with st.expander("📋 View full request details",expanded=False):
                for k,v in af.items():
                    if v and str(v) not in ["","—","False"]: st.markdown(f"**{k}:** {v}")
            # Cancel / recall button (supervisor can cancel their own request).
            if stat not in ("Complete","Cancelled","Declined"):
...
def v_tasks():
    ...
    if recurring:
        st.caption(f"Of your active tasks, {recurring} recurring auto-repeat when completed.")
    ...
    for t in sorted(shown,key=lambda x:x.get("Deadline","")):
        ...
        with st.expander(f"{pri.split(' ')[0]} {t.get('Title','')}  ·  {t.get('Category','')}  ·  {stat}"):
            ...
            # ── Checklist and progress tracker ──
            ...
            if cl_items and isinstance(cl_items,list):
                ...
                pct=int(done_cl/total_cl*100) if total_cl else 0
                pct_color="#27500A" if pct==100 else "#004175" if pct>50 else "#C75B12"
                st.markdown(f'<div style="margin:10px 0 6px;font-size:.8rem;font-weight:700;color:{pct_color};">'
                            f'📋 Checklist progress: {done_cl}/{total_cl} ({pct}%)</div>',unsafe_allow_html=True)
                st.progress(pct/100)
                cl_changed=False
    ...
    # Add-task form goes last, beneath the task lists.
    add_task_form()
...
def v_manage_users():
    ...
    if not active_users:
        ...
        for u in sorted(active_users,key=lambda x:x.get("display_name","")):
            ...
            role_icon={"admin":"🔑","support":"🛠️","supervisor":"👤"}.get(role,"👤")
            with st.expander(f'{role_icon} {u.get("display_name","")} — {u.get("email","")} ({role})'):
                ...
                with nc1:
                    new_role=st.selectbox("Change role",["admin","support","supervisor"],
                        index=["admin","support","supervisor"].index(role),key=f"ur_{uid}")
                with nc2:
                ...
                with nc3:
                    ...
                    if st.button("💾 Save changes",key=f"us_{uid}"):
                        ...
                        if new_pw.strip():
                            ...
                            DB.log_audit(uid,"password_reset","","",cur_user(),st.session_state.user_role)
    ...
    with st.form("add_user_form",clear_on_submit=True):
        ...
        with au3: au_role=st.selectbox(lbl("Role"),["supervisor","support","admin"],
                    help="Supervisor = submit requests & view own status. Support = accept/deny requests, manage tasks. Admin = full access + user management.")
