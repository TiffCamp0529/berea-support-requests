...
def _render_task_detail(t,tid,stat,pri,uc,rec,vis):
    ...
    with qa3:
        if st.button("\U0001f4dd Summary",key=f"qs_{tid}",use_container_width=True):
            st.session_state.open_task_tid=tid  # keep this task's expander open after rerun
            with st.spinner("Generating summary…"):
                st.session_state[f"summary_{tid}"]=generate_task_summary(t)
            st.rerun()
    ...
    if cached_summary:
        ...
        if st.button("Clear summary",key=f"clrsum_{tid}"):
            st.session_state.open_task_tid=tid
            del st.session_state[f"summary_{tid}"]; st.rerun()
