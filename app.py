...
def v_login():
    ...
    with col:
        ...
        email_in=st.text_input("Email address",placeholder="you@berea.edu",key="li_em",label_visibility="visible")
...
def render_gantt(tasks):
    """Draw an SVG timeline: one bar per task from acceptance date → deadline,
    colored by submitter, greyed out when complete. Only tasks appear (a request
    only becomes a task once accepted), so nothing shows before acceptance."""
    from datetime import datetime as _dt, timedelta as _td
    # Parse each task into a drawable row.
    rows=[]
    subs=DB.get_submissions()
    sub_by_ref={s.get("ref_number",""):s for s in subs}
    for t in tasks:
        # Start = acceptance/creation date; End = deadline.
        cs=str(t.get("created_at","")).strip()
        de=str(t.get("Deadline","")).strip()
        start=None; end=None
        # created_at may be "YYYY-MM-DD HH:MM:SS" or "YYYY-MM-DD"; take first 10 chars.
        try: start=_dt.strptime(cs[:10],"%Y-%m-%d")
        except: start=None
        try: end=_dt.strptime(de[:10],"%Y-%m-%d")
        except: end=None
        if not start or not end:
            continue
        # Ensure at least a 1-day bar and end not before start.
        if end<start: end=start
        who=""
        src=t.get("source_ref","")
        if src and src in sub_by_ref:
            who=sub_by_ref[src].get("Your Name","")
    ...
    if not rows:
        st.info("Your timeline will appear here once you've accepted requests or added tasks with deadlines.")
        return

    # Assign a color per submitter (Berea palette + rotation).
    palette=["#004175","#49B4EF","#7D901E","#C75B12","#8AAAC5","#3B6D11","#9b1c1c"]
    whos=[]
    for r in rows:
        if r["who"] not in whos: whos.append(r["who"])
    color_of={w:palette[i%len(palette)] for i,w in enumerate(whos)}

    # Timeline bounds (pad a few days each side).
    from datetime import timedelta as _td2
    min_d=min(r["start"] for r in rows)
    max_d=max(r["end"] for r in rows)
    span=max((max_d-min_d).days,1)
    pad=max(int(span*0.05),2)
    min_d=min_d-_td2(days=pad); max_d=max_d+_td2(days=pad)
    total_days=max((max_d-min_d).days,1)

    # Sort rows by start date, then deadline.
    ...
    parts=[f'<svg viewBox="0 0 {width} {height}" xmlns="http://www.w3.org/2000/svg" '
           f'style="width:100%;height:auto;font-family:Barlow,sans-serif;">']
    ...
    while cur<max_d:
        ...
        # advance one month
        if cur.month==12: cur=_dt(cur.year+1,1,1)
        else: cur=_dt(cur.year,cur.month+1,1)
    ...
    for i,r in enumerate(rows):
        ...
        base=color_of[r["who"]]
        fill="#c9ced6" if r["complete"] else base
        ...
        short_tag={"Confirmed & underway":"Accepted","In progress":"In progress","Awaiting your input":"Awaiting info","On hold":"On hold","Complete":"Complete"}.get(tag,tag)
        if bw>70:
            parts.append(f'<text x="{bx+8:.1f}" y="{y+row_h*0.66:.1f}" font-size="10.5" fill="#ffffff" font-weight="600">{short_tag}</text>')

    # Legend (submitters).
    ly=height-6; lx=left
    parts.append(f'<text x="10" y="{ly:.1f}" font-size="10.5" fill="#5a7a96" font-weight="700">WHO:</text>')
    for w in whos:
        parts.append(f'<rect x="{lx:.1f}" y="{ly-9:.1f}" width="11" height="11" rx="2" fill="{color_of[w]}"/>')
        wl=w if len(w)<=22 else w[:20]+"…"
        parts.append(f'<text x="{lx+15:.1f}" y="{ly:.1f}" font-size="10.5" fill="#333">{wl}</text>')
        lx+=25+len(wl)*6.2
...
def v_tasks():
    ...
    st.caption("Every accepted request and task, drawn from its acceptance date to its deadline. "
               "Bars are colored by who it's for; completed items are greyed out. Updates automatically as you change statuses.")
    render_gantt(tasks)
